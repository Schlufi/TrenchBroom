...
namespace TrenchBroom {
    namespace Renderer {
        ...
        private:
            ...
            private:
                void doRender(PrimType primType, size_t offset, size_t count) const override {
                    glAssert(glDrawElements(toGL(primType), static_cast<GLint>(count), GL_UNSIGNED_INT, reinterpret_cast<void*>(offset)));
                }
            private:
        ...
        public:
            ...
            /**
             * Sets this index array up for rendering. If this index array is only rendered once, then there is no
             * need to call this method (or the corresponding cleanup method), since the render method will perform
             * setup and cleanup automatically unless the index array is already set up when the render method is called.
             *
             * In the case the the index array was already setup before a render method was called, the render method
             ...
             * render calls to this index array.
             */
            bool setup();
