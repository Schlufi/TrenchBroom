...
namespace TrenchBroom {
    namespace Model {
        ...
        const PatchGrid::Point& PatchGrid::point(const size_t row, const size_t col) const {
            return points[row * pointColumnCount + col];
        }
        ...
        std::ostream& operator<<(std::ostream& str, const PatchGrid::Point& p) {
            str << "{ position: {" << p.position << "}, texCoords: {" << p.texCoords << "}, normal: {" << p.normal << "} }";
            return str;
        }
         ...
         *     col
         *
         * For a point at row, col, we compute the normals of the incident quadrants A, B, C, D, take their average, and normalize the result.
         * Not every grid point has four incident quadrants (e.g. the corner points have only one). If the grid points of two opposing sides of the
         * grid coincide, we treat them as one grid point and average their normals.
         */
        std::vector<vm::vec3> computeGridNormals(const std::vector<BezierPatch::Point> patchGrid, const size_t pointRowCount, const size_t pointColumnCount) {
            ...
            enum class RowOffset { Above, Below};
            enum class ColOffset { Left,Right };
             ...
             *     col
             *
             * Quadrant A is above and left of the grid point at row, col, quadrant B is above and right of the grid point, and so on.
             * We determine the incident grid points necessary to compute the normals (via cross product). The returned normal is not normalized.
             */
            ...
            for (size_t col = 1u; col < r; ++col) {
                normals[index(t, col)] =
                    (normalForQuadrant(t, col, RowOffset::Below, ColOffset::Left) + 
                     normalForQuadrant(t, col, RowOffset::Below, ColOffset::Right)) / static_cast<FloatType>(2);
                normals[index(b, col)] =
                    (normalForQuadrant(b, col, RowOffset::Above, ColOffset::Left) + 
                     normalForQuadrant(b, col, RowOffset::Above, ColOffset::Right)) / static_cast<FloatType>(2);
            }
            ...
            for (size_t row = 1u; row < b; ++row) {
                normals[index(row, l)] =
                    (normalForQuadrant(row, l, RowOffset::Above, ColOffset::Right) + 
                     normalForQuadrant(row, l, RowOffset::Below, ColOffset::Right)) / static_cast<FloatType>(2);
                normals[index(row, r)] =
                    (normalForQuadrant(row, r, RowOffset::Above, ColOffset::Left) + 
                     normalForQuadrant(row, r, RowOffset::Below, ColOffset::Left)) / static_cast<FloatType>(2);
            }
        ...
        PatchGrid makePatchGrid(const BezierPatch& patch, const size_t subdivisionsPerSurface) {
            const size_t gridPointRowCount = patch.surfaceRowCount() * (1u << subdivisionsPerSurface) + 1u;
            const size_t gridPointColumnCount = patch.surfaceColumnCount() * (1u << subdivisionsPerSurface) + 1u;
            ...
            for (const auto [point, normal] : kdl::make_zip_range(patchGrid, normals)) {
                points.push_back(PatchGrid::Point{vm::slice<3>(point, 0), vm::slice<2>(point, 3), normal});
            }

            return {
                gridPointRowCount,
                gridPointColumnCount,
                std::move(points)
            };
        }
        ...
        void PatchNode::doPick(const vm::ray3& pickRay, PickResult& pickResult) {
            const auto pickTriangle = [&](const auto& v0, const auto& v1, const auto& v2) {
                if (const auto distance = vm::intersect_ray_triangle(pickRay, v0, v1, v2); !vm::is_nan(distance)) {
        ...
        bool PatchNode::doContains(const Node*) const {
            return false;
        }

        bool PatchNode::doIntersects(const Node*) const {
            return false;
        }
